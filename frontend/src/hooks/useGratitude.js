  ...
  const {
    ...
    queryFn: async () => {
      try {
        ...
        console.log(
          `Fetched ${data.length} gratitude entries for the current user`
        );
        return data;
      } catch (error) {
  ...
  useEffect(() => {
    ...
    if (todayEntry && todayEntry.id) {
      try {
        ...
        if (
          progressData?.completedTasks?.gratitude ||
  ...
  const {
    ...
    onMutate: async (data) => {
      ...
      await queryClient.cancelQueries({
        queryKey: ["gratitude", getAuthToken()],
      });

      // Snapshot the previous entries
      const previousEntries =
        queryClient.getQueryData(["gratitude", getAuthToken()]) || [];
      ...
      queryClient.setQueryData(["gratitude", getAuthToken()], (old) => [
        optimisticEntry,
        ...(old || []),
      ]);
