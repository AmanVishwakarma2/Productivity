      ...
      try {
        ...
        try {
          ...
          if (response.status === 404) {
            console.log(
              "Auth endpoint not found, trying alternative endpoint..."
            );
          ...
          if (
            ...
            console.warn(
              "Network error, but keeping user logged in with stored credentials"
            );
          }
        }
      } catch (error) {
  ...
  const logoutMutation = useMutation({
    mutationFn: async () => {
      try {
        ...
        console.warn(
          "Error during logout request, but proceeding with local logout:",
          error
        );
      }
    ...
    <AuthContext.Provider
      value={{
        ...
        register: (userData, options) =>
          registerMutation.mutate(userData, options),
