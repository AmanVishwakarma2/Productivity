...
export const useTodo = () => {
  ...
  const updateProgress = useCallback(
    (taskType, isCompleted) => {
      console.log(
        `Updating progress for ${taskType} with completion status: ${isCompleted}`
      );
  ...
  const saveAllTodosMutation = useMutation({
      ...
      const promises = completedTodos.map((todo) =>
        fetch(`${API_URL}/todo`, {
          ...
          if (data && data._id && data.completed === false) {
            ...
            return fetch(`${API_URL}/todo/${data._id}`, {
              method: "PUT",
              headers: {
                "Content-Type": "application/json",
              },
    ...
    onSuccess: (data) => {
      ...
      setSuccessMessage(
        "All todos saved successfully and stored in the database!"
      );
  ...
  const toggleTodo = useCallback(
    (id, currentStatus) => {
      console.log(
        `Toggle todo ${id} from ${currentStatus} to ${!currentStatus}`
      );
  ...
  const markAllAsCompleted = useCallback(() => {
    ...
    Promise.all(
      ...
      .then(() => {
        ...
        queryClient.invalidateQueries({ queryKey: ["progress"] });
        setSuccessMessage(
          `Fixed ${incompleteTodos.length} incomplete todo${incompleteTodos.length > 1 ? "s" : ""}!`
        );
        setTimeout(() => setSuccessMessage(""), 3000);
