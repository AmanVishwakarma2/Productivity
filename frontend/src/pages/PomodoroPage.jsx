...
const ForceBreakModal = ({ timeLeft, mode, onBreakComplete }) => {
  ...
  function getBreakPercentage() {
    ...
    console.log(
      `Break progress: ${progress.toFixed(1)}% (${timeLeft}s / ${totalBreakTime}s)`
    );
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-8 max-w-md w-full dark:bg-gray-800 ">
        ...
        <p className="mb-6 text-center">
          You've completed a {mode} Pomodoro session. Now you must take a{" "}
          {getBreakName()} break before continuing.
        </p>
        ...
        <div className="text-center py-6">
          ...
          <p className="text-sm text-gray-500 dark:text-white">
            Please take this time to rest your eyes, stretch, or grab a drink of
            water.
            <br />
...
function PomodoroPage() {
  ...
  const [timeLeft, setTimeLeft] = useState(() => {
    ...
    if (savedState && savedState.userId === user?.id) {
      console.log(
        "Loading saved timer state with time:",
        calculateRemainingTime(savedState)
      );
  ...
  const [isRunning, setIsRunning] = useState(() => {
    const savedState = getSavedTimerState();
    return savedState && savedState.userId === user?.id
  ...
  const [isWorkTime, setIsWorkTime] = useState(() => {
    const savedState = getSavedTimerState();
    return savedState && savedState.userId === user?.id
      ? savedState.isWorkTime !== false
      : true;
  });
  const [sessionCompleted, setSessionCompleted] = useState(() => {
    const savedState = getSavedTimerState();
    return savedState && savedState.userId === user?.id
      ? savedState.sessionCompleted || false
      : false;
  });
  ...
  const [completePomodoroOnBreakEnd, setCompletePomodoroOnBreakEnd] =
    useState(false);
  ...
  useEffect(() => {
    ...
    const hasBreakData =
      localStorage.getItem(FORCED_BREAK_KEY) ||
    ...
    if ((isBreakInProgress || hasBreakData) && !isWorkTime) {
      ...
      if (location !== "/pomodoro") {
        console.log(
          "Break detected during initialization, navigating to pomodoro"
        );
        navigate("/pomodoro");
      }
      ...
      try {
        ...
        const endTime =
          localStorage.getItem("break_end_time") ||
          sessionStorage.getItem("break_end_time");
        ...
        if (endTime) {
          ...
          breakTimeRemaining =
            parseInt(localStorage.getItem("break_time_left")) ||
            getBreakDuration(breakMode);
          ...
          if (lastSaved) {
            ...
            breakTimeRemaining = Math.max(
              0,
              breakTimeRemaining - elapsedSeconds
            );
        ...
        console.log(
          "Break state recovered during initialization with",
          breakTimeRemaining,
          "seconds left"
        );
        ...
        console.error(
          "Error recovering break state during initialization:",
          error
        );
    ...
    const setupGlobalBreakCheck = () => {
      ...
      if (sessionStorage.getItem("checked_break_on_startup") !== "true") {
        ...
        const checkBreakSources = () => {
          ...
          const userBreakKey = user?.id
            ? `${FORCED_BREAK_KEY}_${user.id}`
            : null;
          const userBreakData = userBreakKey
            ? localStorage.getItem(userBreakKey)
            : null;

          // Check session storage flag
          const sessionBreakFlag =
            sessionStorage.getItem("break_in_progress") === "true";
          const localBreakFlag =
            localStorage.getItem("break_in_progress") === "true";
          ...
          if (
            ...
            try {
              ...
              // Then check session storage
                ...
                expiryTime = parseInt(
                  sessionStorage.getItem("break_expiry_time")
                );
                isActive = now < expiryTime;
              }
              ...
              // Check if break started within last 24 hours
                ...
                if (isActive && !expiryTime) {
                  ...
                  console.log(
                    "Break from within 24 hours detected, resuming with full duration"
                  );
              ...
              if (isActive || sessionBreakFlag || localBreakFlag) {
                console.log(
                  "Active break detected, redirecting to pomodoro page"
                );
                if (location !== "/pomodoro") {
                  navigate("/pomodoro");
                }
    ...
    const handleNavigation = () => {
      ...
      if (!showForcedBreak) {
        ...
        if (breakInProgress && location !== "/pomodoro") {
          console.log(
            "Navigation detected during break, redirecting to pomodoro page"
          );
          navigate("/pomodoro");
          return true;
        }
    ...
    const handleFocus = () => {
      ...
      if (!showForcedBreak) {
        ...
        if (
          ...
          console.log(
            "Break in progress detected on focus, redirecting to pomodoro page"
          );
          navigate("/pomodoro");
          checkForcedBreak();
          return;
        ...
        if (breakData && location !== "/pomodoro") {
          try {
            ...
            const breakStartTime =
              startTime || expiryTime - getBreakDuration(mode) * 1000;

            if (now < expiryTime || breakStartTime > twentyFourHoursAgo) {
              console.log(
                "Active break detected on focus, redirecting to pomodoro page"
              );
              navigate("/pomodoro");
        ...
        if (existingEndTime && breakInProgress) {
          ...
          if (endTimeMs > now) {
            ...
            if (currentTimeLeft > 0) {
              console.log(
                `Resuming break with ${currentTimeLeft} seconds left`
              );
              const savedMode =
                localStorage.getItem("last_pomodoro_mode") || mode;
              setMode(savedMode);
  ...
  const startBreakTimer = (initialTime) => {
    ...
    breakTimerRef.current = setInterval(() => {
      ...
      const exactTimeRemaining = Math.max(
        0,
        Math.ceil((endTimeMs - currentTime) / 1000)
      );
  ...
  const startForcedBreak = (() => {
    ...
    return (breakMode) => {
      ...
      console.log(
        `Starting forced break for mode: ${breakMode}, cycle: ${cycle}`
      );
      ...
      const userSpecificBreakKey = user?.id
        ? `${FORCED_BREAK_KEY}_${user.id}`
        : FORCED_BREAK_KEY;
  ...
  const completeBreak = () => {
    ...
    try {
      ...
      if (progressData) {
        ...
        console.log(
          "Directly updated localStorage progress data after break completion"
        );
      }
    } catch (error) {
    ...
    try {
      ...
      if (state.isRunning && state.expiryTimestamp) {
        ...
        const timeRemaining = Math.max(
          0,
          Math.ceil((state.expiryTimestamp - now) / 1000)
        );
  ...
  const calculateProgress = useCallback(() => {
    ...
    const progress = Math.min(
      100,
      Math.max(0, ((cycle - 1) / maxCycles) * 100)
    );
    return progress;
  }, [cycle, mode]);
          ...
          if (prev <= 1) {
            ...
            if (isWorkTime) {
              ...
              updateTaskCompletionMutation.mutate(
                ...
                },
                {
                  onSuccess: () => {
                    ...
                    setTimeout(() => refreshProgress(), 500);
                    console.log(
                      "Progress bar updated after completing pomodoro cycle"
                    );
                  },
                }
              );
              ...
              const streakProgress =
                localStorage.getItem("streak_progress") || "0";
              ...
              if (newProgress >= 100) {
                ...
                const currentStreak = parseInt(
                  localStorage.getItem("current_streak") || "0"
                );
                localStorage.setItem(
                  "current_streak",
                  (currentStreak + 1).toString()
                );
              }

              console.log(
                `Work session complete, starting break for ${mode} mode`
              );
        ...
        if (savedCompletion) {
          try {
            ...
            if (timestamp) {
              ...
              if (!isSameDay) {
                console.log(
                  `Resetting cycles for ${modeType} - new day detected`
                );
  ...
  const markPomodoroCompleted = (mode, isLastCycle, isBreak = false) => {
    ...
    console.log(
      `Marking pomodoro completed. Mode: ${mode}, Cycle: ${cycle} -> ${newCycle}, Max: ${maxCycles}`
    );

    // Check if this completes all cycles for this mode
    if (isCompleted) {
      console.log(
        `All ${maxCycles} cycles completed for ${mode}. Marking as completed.`
      );
    ...
    completePomodoroMutation.mutate(
      ...
      },
      {
        onSuccess: () => {
          ...
          refreshProgress();
          console.log(
            "Progress bar updated after completing pomodoro in database"
          );
        },
      }
    );
  ...
  const updateProgressBar = useCallback(() => {
    ...
    try {
      ...
      if (progressData) {
        ...
        console.log(
          "Direct localStorage update for faster progress bar by updateProgressBar()"
        );
      }
    } catch (err) {
      console.error("Error updating progress data in localStorage:", err);
        ...
        if (breakActive) {
          ...
          if (storedEndTime) {
            ...
            if (endTimeMs > now) {
              const timeLeftSeconds = Math.ceil((endTimeMs - now) / 1000);
              console.log(
                `Updating break UI with ${timeLeftSeconds}s remaining`
              );
              setBreakTimeLeft(timeLeftSeconds);
    ...
    <div className="min-h-screen bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white">
      ...
      <main className="container mx-auto px-4 py-8 dark:bg-gray-800 text-gray-900 dark:text-white hover:shadow-lg transition-shadow shadow-2xl">
        ...
        <div className="bg-white border rounded-lg p-6 dark:bg-gray-800 text-gray-900 dark:border-black dark:text-white shadow-2xl">
          <h2 className="text-xl font-semibold mb-4 hover:shadow-lg transition-shadow shadow-2xl">
            Pomodoro Timer
          </h2>
          ...
          <div className="text-center py-8 hover:shadow-lg transition-shadow shadow-2xl">
            ...
            <div className="text-gray-500 mb-6 dark:text-white">
              {isWorkTime ? "Work Time" : "Break Time"} - Cycle {cycle}/
              {getMaxCycles(mode)}
            </div>
            ...
            {cycle > getMaxCycles(mode) && !showForcedBreak && (
              <div className="mt-6 p-3 bg-blue-100 text-blue-700 dark:bg-blue-500 rounded hover:shadow-lg transition-shadow shadow-2xl">
                You've completed all {getMaxCycles(mode)} cycles for {mode}{" "}
                mode! You've done enough for today - feel free to focus on other
                things.
              </div>
            )}
